...
cartoon effect (anime style), and many other features.
"""
...
# CARTOON EFFECT
# ============================================================================
def apply_cartoon_effect(img, intensity=0.5, smooth_radius=2.0, bits=4):
    """
    Apply an anime/cartoon style effect.
    - intensity: edge darkening strength (0.0 to 1.0)
    - smooth_radius: Gaussian blur radius for smoothing (larger = more painterly)
    - bits: posterization bits per channel (lower = fewer colors)
    """
    if intensity <= 0:
        return img

    # Work on RGB (discard alpha for the effect, we'll restore it later)
    ...
    # Smooth
    if smooth_radius > 0:
        smoothed = rgb.filter(ImageFilter.GaussianBlur(radius=smooth_radius))
    else:
        smoothed = rgb
    ...
    edges = ImageOps.invert(edges)      # white edges on black

    # Create a mask: white lines to overlay black
    edge_mask = edges  # white lines

    # Overlay black on posterized where edges are strong
    black = Image.new('RGB', posterized.size, (0, 0, 0))
    result = Image.composite(black, posterized, edge_mask)

    # Blend with posterized using intensity
...
class AdvancedWallpaperGenerator:
    ...
    def generate_from_config(self, config: Dict[str, Any]) -> Image.Image:
        ...
        # ======================== CARTOON EFFECT =========================
        # Apply cartoon effect before logo and global effects
        if config.get("cartoon", False):
            # Get parameters from config or use defaults
            intensity = config.get("cartoon_intensity", 0.5)
            bits = config.get("cartoon_bits", 4)
            smooth = config.get("cartoon_smooth", 2.0)
            img = apply_cartoon_effect(img, intensity, smooth, bits)
...
def parse_arguments():
    ...
    parser.add_argument("--cartoon", action="store_true",
                        help="Apply cartoon/anime style effect")
    parser.add_argument("--cartoon-intensity", type=float, default=0.5,
                        help="Edge darkening intensity for cartoon effect (0.0 to 1.0)")
    parser.add_argument("--cartoon-bits", type=int, default=4,
                        help="Posterization bits per channel (lower = fewer colors, more cartoonish, min=2)")
    parser.add_argument("--cartoon-smooth", type=float, default=2.0,
                        help="Smoothing radius before posterization (larger = more painterly)")

    parser.add_argument("--log-level", default="INFO", choices=["DEBUG","INFO","WARNING","ERROR"])
    parser.add_argument("--version", action="version", version="LFS Wallpaper Generator v3.1")
...
def main():
    ...
    for cfg in selected:
        ...
        # Override cartoon settings if provided
        if args.cartoon:
            cfg["cartoon"] = True
            cfg["cartoon_intensity"] = args.cartoon_intensity
            cfg["cartoon_bits"] = args.cartoon_bits
            cfg["cartoon_smooth"] = args.cartoon_smooth
        # Other overrides
        if args.watermark:
